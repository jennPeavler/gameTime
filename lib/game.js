...
class Game {
  ...
  draw () {
    this.paddle.draw();
    this.ball.draw();
    this.bricks.place(this.bricks.brick);
    this.drawScore();
    this.drawLevel();
    this.drawLives();
  }
    ...
    for (let c = 0; c < this.bricks.columns; c++) {
      for (let r = 0; r < this.bricks.rows; r++) {
        ...
        if (brickInQuestion.status === 1) {
          if (nextX + ballRad > brickInQuestion.x &&
              nextX - ballRad < brickInQuestion.x + this.bricks.width &&
              nextY + ballRad > brickInQuestion.y &&
              nextY - ballRad < brickInQuestion.y + this.bricks.height)
          {
            this.ball.dy = -this.ball.dy;
